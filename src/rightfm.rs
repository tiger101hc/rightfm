...
use tqdm::Iter;
use crate::rightfm_fast::{FastRightFM, fit_bpr, fit_logistic, fit_warp, fit_warp_kos, predict_ranks, predict_rightfm};
...
impl RightFM {
    ...
    pub fn fit(&mut self, interaction: &CooMatrix<Flt>,
               user_features: Option<CsrMatrix<Flt>>, item_features: Option<CsrMatrix<Flt>>,
    ...
    pub fn fit_partial(&mut self, interaction: &CooMatrix<Flt>, user_features: Option<CsrMatrix<Flt>>, item_features: Option<CsrMatrix<Flt>>,
                       sample_weight: Option<&CooMatrix<Flt>>, epochs: usize, number_thread: u8, verbose: bool) {
        let (no_user, no_item) = (interaction.nrows(), interaction.ncols());
        let _user_features = if user_features.is_some() { user_features.unwrap() } else { CsrMatrix::<Flt>::identity(no_user) };
        let _item_features = if item_features.is_some() { item_features.unwrap() } else { CsrMatrix::<Flt>::identity(no_item) };
        assert!(interaction.nrows() >= _user_features.nrows(), "interaction matrix must have more rows than user features matrix");
        assert!(interaction.ncols() >= _item_features.nrows(), "interaction matrix must have more cols than item features matrix");
        let sample_weight_data = self.process_sample_weight(interaction, sample_weight);
        if self.data.is_none() {
            self.data = Some(
                FastRightFM::new(self.no_components, _item_features.ncols(), _user_features.ncols(), self.learning_schedule.clone(), self.learning_rate, self.rho, self.epsilon, self.max_sampled)
            );
        } else {
            assert_eq!(self.data.as_ref().unwrap().item_features.nrows(), _item_features.ncols(), "should have same no of item features");
    ...
    fn process_sample_weight(&self, interaction: &CooMatrix<Flt>, sample_weight: Option<&CooMatrix<Flt>>) -> Vec<Flt> {
        if sample_weight.is_some() {
            ...
            assert!(data.row_indices() == interaction.row_indices() && data.col_indices() == interaction.col_indices(), "sample weight and interactions matrices must be the same shape and same order");
            return data.values().to_vec();
        }
        return Array::ones(interaction.nnz()).to_vec();
    }

    pub fn predict(&mut self, user_ids: &[usize], item_ids: &[usize], user_features: Option<CsrMatrix<Flt>>, item_features: Option<CsrMatrix<Flt>>) -> Vec<Flt> {
        ...
        let _user_features = if user_features.is_some() { user_features.unwrap() } else { CsrMatrix::<Flt>::identity(no_users) };
        let _item_features = if item_features.is_some() { item_features.unwrap() } else { CsrMatrix::<Flt>::identity(no_items) };
        assert_eq!(no_users, _user_features.nrows(), "user_ids and user_features must have the same no of users");
        assert_eq!(no_items, _item_features.nrows(), "item_ids and item_features must have the same no of items");
        if self.data.is_none() {
            self.data = Some(
                FastRightFM::new(self.no_components, _item_features.ncols(), _user_features.ncols(), self.learning_schedule.clone(), self.learning_rate, self.rho, self.epsilon, self.max_sampled)
            );
        ...
        assert_eq!(_item_features.ncols(), _data.item_features.shape()[0], "no. of item features must have the same shape[0] to item embedded");
        assert_eq!(_user_features.ncols(), _data.user_features.shape()[0], "no. of user features must have the same shape[0] to user embedded");
        let mut predictions = vec![0.; user_ids.len()];
        predict_rightfm(&_user_features, &_item_features, user_ids, item_ids, &mut predictions, _data, 1);
        predictions
    }
    ...
    pub fn predict_rank(
        ...
        assert!(self.data.is_some(), "FastRightFM model must be fitted before calling predict_rank");
        let mut _train_interactions =
            if check_intersections && train_interactions.is_some() {
                let _t = train_interactions.unwrap();
                let n_intersections = elementwise_multiply(&_t, &test_interactions).nnz();
                assert_eq!(n_intersections, 0, "train_interactions and test_interactions must not have any intersections");
                Some(_t)
            } else {
                train_interactions
            };
        let (n_users, n_items) = (test_interactions.nrows(), test_interactions.ncols());
        let _user_features = if user_features.is_some() { user_features.unwrap() } else { CsrMatrix::<Flt>::identity(n_users) };
        let _item_features = if item_features.is_some() { item_features.unwrap() } else { CsrMatrix::<Flt>::identity(n_items) };
        let _data = self.data.as_ref().unwrap();
        assert_eq!(_item_features.ncols(), _data.item_features.shape()[0], "no. of item features must have the same shape[0] to item embedded");
        assert_eq!(_user_features.ncols(), _data.user_features.shape()[0], "no. of user features must have the same shape[0] to user embedded");
        if _train_interactions.is_none() { _train_interactions = Some(CsrMatrix::zeros(n_users, n_items)); }
        let mut ranks = vec![0.; test_interactions.nnz()];
        predict_ranks(&_user_features, &_item_features, &test_interactions, _train_interactions.as_ref().unwrap(), &mut ranks, _data, num_threads);
        CsrMatrix::try_from_csr_data(n_users, n_items, test_interactions.row_offsets().to_vec(), test_interactions.col_indices().to_vec(), ranks).unwrap()
    }
}

fn elementwise_multiply(a: &CsrMatrix<Flt>, b: &CsrMatrix<Flt>) -> CsrMatrix<Flt> {
    assert!(a.nrows() == b.nrows() && a.ncols() == b.ncols(), "they must have same shape");
